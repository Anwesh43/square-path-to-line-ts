...
class SPTLNode {
    ...
    addNeighbor() {
        if (this.i < colors.length - 1) {
            this.next = new SPTLNode(this.i + 1)
            this.next.prev = this 
        }
    }

    draw(context : CanvasRenderingContext2D) {
        DrawingUtil.drawSPTLNode(context, this.i, this.state.scale)
    }
    ...
    getNext(dir : number, cb : Function) : SPTLNode {
        var curr : SPTLNode = this.prev 
        if (dir == 1) {
            curr = this.next 
        }
        if (curr) {
...
class SquarePathToLine {

    curr : SPTLNode = new SPTLNode(0)
    dir : number = 1
